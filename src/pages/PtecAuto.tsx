...
const PtecAuto = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingOS) {
      ...
      const { error } = await supabase.from("ptec_auto_os").insert([
        {
          ...dataToSubmit,
  ...
  return (
    <div>
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground mb-2">Ptec Auto</h1>
          <p className="text-muted-foreground">
            Companhia de Manutenção Automotiva
          </p>
        </div>

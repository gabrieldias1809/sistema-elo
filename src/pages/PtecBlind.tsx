...
const PtecBlind = () => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel("ptec_blind_os_changes")
      .on("postgres_changes", { event: "*", schema: "public", table: "ptec_blind_os" }, () => {
        fetchOS();
      })
      .subscribe();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingOS) {
      ...
      const { error } = await supabase.from("ptec_blind_os").insert([
        {
          ...dataToSubmit,
